...
const DEFAULT_STOP_AFTER_ERRORS = 3;
const PROVINCES = [
...
async function fetchJson(url, retry = DEFAULT_RETRY_COUNT) {
  let lastError = null;
  ...
  const reason = lastError?.name === "AbortError"
    ? "요청 시간이 초과되었습니다"
    : (lastError?.message || "알 수 없는 네트워크 오류");

  throw new Error(`${reason} · 브라우저가 Hugging Face 요청을 차단했거나, 일시적인 네트워크/속도 제한 문제가 발생했을 수 있습니다.`);
}
...
function getCriteria() {
  ...
  const requestDelay = clampInt($("requestDelay").value, 0, 5000, DEFAULT_REQUEST_DELAY_MS);
  const retryCount = clampInt($("retryCount").value, 0, 10, DEFAULT_RETRY_COUNT);
  const stopAfterErrors = clampInt($("stopAfterErrors").value, 1, 20, DEFAULT_STOP_AFTER_ERRORS);
...
async function runSearch() {
  ...
  state.lastOffset = criteria.startOffset;
  state.lastError = "";
  ...
  log(`안정 모드: 요청 간 지연 ${criteria.requestDelay}ms / 재시도 ${criteria.retryCount}회 / 연속 오류 ${criteria.stopAfterErrors}회 시 중단`);
  ...
  try {
    ...
    while (!state.stopRequested && state.matched.length < criteria.targetCount && offset < endOffset) {
      ...
      let data = null;
      try {
        data = await fetchJson(url, criteria.retryCount);
        state.apiCalls += 1;
        consecutiveErrors = 0;
      } catch (err) {
        consecutiveErrors += 1;
        ...
        log(`API 호출 실패: offset ${offset.toLocaleString()} / ${err.message}`);

        if (state.matched.length > 0) {
          log(`현재까지 수집된 ${state.matched.length.toLocaleString()}명은 엑셀로 저장할 수 있습니다.`);
        }

        if (consecutiveErrors >= criteria.stopAfterErrors) {
          log(`연속 오류 ${consecutiveErrors}회로 검색을 중단했습니다. 요청 간 지연을 800~1200ms로 높이거나 시작 위치를 바꿔 다시 실행해 주세요.`);
          break;
        }

        offset += length;
        updateProgress();
        await sleep(Math.max(criteria.requestDelay, 1200) * consecutiveErrors);
        continue;
      }
      ...
      await sleep(criteria.requestDelay);
    }
    ...
    if (state.matched.length > 0) {
      ...
      log(`완료/중단 시점: 수집 ${state.matched.length.toLocaleString()}명, 검색 ${state.scanned.toLocaleString()}행, API 성공 ${state.apiCalls.toLocaleString()}회, 실패 배치 ${state.failedBatches.toLocaleString()}회`);
    }
  }
}
...
function updateProgress() {
  ...
  if (failedEl) failedEl.textContent = state.failedBatches.toLocaleString();
...
function backoffMs(attempt) {
  const base = 900 * Math.pow(1.7, attempt);
  const jitter = Math.floor(Math.random() * 350);
  return Math.min(12000, Math.round(base + jitter));
}
